...
def fetch_items(base: str, ch_cfg: dict, token: str, token_prefix: str):
...
def main() -> int:
    ...
    if not token:
        log(f"[chall-agent] ✗ 未找到平台令牌：请设置环境变量 {token_env}")
        print("FETCH DONE registered=0 skipped=0 reason=no-token")
        return 1
    ...
    try:
        items = fetch_items(base, ch_cfg, token, prefix)
    ...
    for it in items:
        ...
        category = str(field(it, m.get("category", "category")) or "misc").lower()
