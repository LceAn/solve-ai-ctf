...
async function renderEnvTab(target = "#envBody") {
  const s = ENV_DATA;
  if (!s) return;
  const body = $("#envTabBody");
  if (ENV_TAB === "registry") return renderEnvRegistry(body);
  if (ENV_TAB === "pool") return renderEnvPool(body, s);
...
function renderEnvPool(body, s) {
  ...
  $$("#envTabBody [data-envrebuild]").forEach((b) =>
    b.onclick = () => dispatch("/api/env/build",
      { preheat: true, categories: b.dataset.envrebuild, rebuild: true },
      `重建 ${b.dataset.envrebuild} 层`)());
  const pre = $("#envPoolPreheat"), cl = $("#envPoolClean");
...
/* ---- 环境页签 3：仓库推送 ---- */
async function renderEnvRegistry(body) {
  ...
  $("#registrySave").onclick = async () => {
    ...
    if (r.ok) { toast("仓库地址已保存 ✓"); renderEnvRegistry(body); }
    else toast(r.error || "保存失败", true);
  };
  const doPush = (payload, label) => async () => {
    const r = await fetch("/api/env/push", { method: "POST",
      headers: authHeaders({ "Content-Type": "application/json" }),
      body: JSON.stringify({ dir: S.dir, ...payload }) }).then((x) => x.json()).catch((e) => ({ ok: false, error: String(e) }));
    if (r.ok) { toast(`${label} 已派发 ✓（任务 ${r.task.id}）`); TaskUI.selected = r.task.id; }
